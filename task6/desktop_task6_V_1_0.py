...
def count_ks(G, map_me, edge_col, node_col, arr_color):
    ...
    for i in range(id_start, len(all_v)):
        if all_v[i] != -1:
            ...
            ans += 1
    try:
        if len(entry_answer.get().replace(' ', '')) == 0:
            tk.messagebox.showwarning(title='Оййй, не знал)', message='Поле количество компонент связности должно быть заполнено!')
            return
        if ans != int(entry_answer.get()):
            message_ask = tk.messagebox.askyesno(title='Оййй, не знал)', message=f'Ваш ответ неверный!\nПоказать правильный?')
            if message_ask:
                animation_graph_bfs(G, node_col, edge_col, ans)
            return
    except Exception:
        tk.messagebox.showwarning(title='Оййй, не знал)', message='Вводите только цифры!')
        return
    animation_graph_bfs(G, node_col, edge_col, ans)
...
def input_ss():
    ...
    label_name_ss_ms_mi_win = top_canvas.create_window(456, 55, window=label_name_ss_ms_mi)
...
##    node_number_button.current(0)
    ...
    entry_answer = Entry(width=20, font=('Arial', 12, 'normal'), justify='center')
    top_canvas.create_window(550, 20, window=entry_answer)
...
##    node_number_button_bfs.current(0)
    ...
    label_v_bfs2 = Label(text='Количество компонент связности', font=('Arial', 12, 'normal'))
    label_v_bfs2_win = top_canvas.create_window(320, 20, window=label_v_bfs2)
...
def input_ms():
    ...
    label_name_ss_ms_mi_win = top_canvas.create_window(463, 55, window=label_name_ss_ms_mi)
...
def input_mi():
    ...
    label_name_ss_ms_mi_win = top_canvas.create_window(558, 55, window=label_name_ss_ms_mi)
...
##    node_number_button.current(0)
    ...
    entry_answer = Entry(width=41, font=('Arial', 12, 'normal'), justify='center')
    top_canvas.create_window(647, 20, window=entry_answer)
...
label_name_ss_ms_mi = Label(text='SS MS MI', font=('Arial', 12, 'normal'))
label_name_ss_ms_mi_win = top_canvas.create_window(500, 55, window=label_name_ss_ms_mi)
