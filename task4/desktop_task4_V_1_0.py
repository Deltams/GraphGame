...
def check_user_input(map_me, arr_user):
    # Проверка "св-ва" для обхода в ширину
    ...
    for i in map_me:
        if not i in arr_user:
            return False
    # Проверяем обход
    array = bfs(map_me, arr_user[0])
    now_bfs = 1
    for i in range(1, len(map_me)):
        true_array = []
        for j in array:
            if j[2] == i:
                true_array.append(j[1])
        if len(true_array) == 0:
            continue
        for j in true_array:
            check = True
            for q in range(len(true_array)):
                if arr_user[q+now_bfs] == j:
                    check = False
                    break
            if check:
                print('p5')
                return False
        now_bfs += len(true_array)
    return True
...
def task1_start():
    ...
    if not check_user_input(map_me, arr):
        ...
        if message_ask:
            ...
            animation_graph(G, node_col, edge_col, point_bfs, arr, 1)
        return
    
    arr = bfs(map_me, arr[0])
...
##    node_number_button_bfs.current(0)
    ...
    label_v_bfs2 = Label(text='Введите результат обхода в ширину', font=('Arial', 12, 'normal'))
    label_v_bfs2_win = top_canvas.create_window(150, 55, window=label_v_bfs2)
