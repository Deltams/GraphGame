    ...
    while True:
        ...
        for i in range(len(arr_v)):
            if check_arr_v[i] == 0:
                ...
                if check_for:
                    continue
                arr_anim.append([arr_v[i][1], col])
                #set_color_node(G, node_col, arr_v[i][1], arr_color[col])
                check_col_v[arr_v[i][1]] = step
                check_arr_v[i] = step
        col += 1
        step += 1
    animation_graph_bfs(G, node_col, edge_col, arr_color, arr_anim, 1)

# Проверка данных в map_me
def valid_map_me(map_me):
...
def animation_graph_bfs(G, node_col, edge_col, arr_color, arr_anim, step):
    global pos
    if len(arr_anim)+1 == step:
        print(len(arr_anim))
        return
    plt.clf()
    if step == 1:
        ...
        pos=pos
    set_color_node(G, node_col, arr_anim[step-1][0], arr_color[arr_anim[step-1][1]])
    nx.draw_networkx(G, pos, with_labels=True, font_weight='bold', node_color=node_col, edge_color=edge_col, width=2)
    plt.legend(labels=[str(arr_anim[step-1][1]+1)], loc='best', fontsize=12, shadow=True, framealpha=1, facecolor='#08E8DE', edgecolor='#40E0D0', title='Всего цветов использовано')
    plt.show(block=False)
    root.after(500, lambda: animation_graph_bfs(G, node_col, edge_col, arr_color, arr_anim, step+1))
