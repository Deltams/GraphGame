...
class PlayTool(ToolBase):
    ...
    def trigger(self, *args, **kwargs):
        ...
        for i in map_me:
            if i > ch:
                ch = i
        node_col_anim = set_color_node(G_anim, node_col_anim, ch, 'yellow')
        for i in range(step_pos):
            node_col_anim = set_color_node(G_anim, node_col_anim, arr_anim[i][1], 'yellow')
            edge_col_anim = set_color_edge(G_anim, edge_col_anim, arr_anim[i], 'red')
        animation_graph(G_anim, node_col_anim, edge_col_anim, arr_anim, step_pos)
    ...
    def trigger(self, *args, **kwargs):
        ...
        if len(arr_anim) <= step_pos:
            ...
            for i in map_me:
                Gn.add_node(i)
            for i in arr_anim:
                if not (i[0], i[1]) in edge_lab:
                    Gn.add_edge(i[1], i[0], weight=edge_lab[(i[1], i[0])])
                else:
                    Gn.add_edge(i[0], i[1], weight=edge_lab[(i[0], i[1])])
            edge_lab = nx.get_edge_attributes(Gn,'weight')
            nd_col = ['yellow']*(len(edge_lab)+1)
            ed_col = ['red']*len(edge_lab)
            plt.clf()
            nx.draw_networkx(Gn, pos, with_labels=True, font_weight='bold', node_color=nd_col, edge_color=ed_col, width=2)
            nx.draw_networkx_edge_labels(Gn,pos,edge_labels=edge_lab)
            plt.show(block=False)
            check_leftArrow = True
            stop_animation = True
            return
...
def animation_graph(G, node_col, edge_col, anim_arr, step):
    global pos, stop_animation, step_pos, check_leftArrow, map_me
    if step == 0:
        step = 1
...
def algo_prim(G, map_me, edge_col, node_col):
    ...
    node_col_anim = set_color_node(G, node_col, list(visit_v)[0], 'yellow')
...
##        set_color_edge(G, edge_col, r, 'red')
    ...
    arr_anim = anim_arr
    animation_graph(G, node_col, edge_col, anim_arr, 0)
    add_tool()
